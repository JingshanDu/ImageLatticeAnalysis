function out = scatplot_pbc(x,y,method,radius,N,n,po,ms,scale,lenA,lenB,angle)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Modified to consider the periodic boundary conditions in the segmented
% unit cell images. Jingshan S. Du du@u.northwestern.edu
% --lenA, lenB, angle: the unit cell parameters. If any is empty, we assume
% that the image boundaries are the field boundaries.
% --scale: color scale from density = 0 to how many times of the standard
% density. default is 2.
% The original information of this function is as follows:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if nargin<9 || isempty(scale)
    scale = 2; %scale of color
end
if nargin<10 || isempty(lenA) || isempty(angle)
    blnIsParagrm = 0;
else
    blnIsParagrm = 1;
    if strcmp(method, 'vo')
        fprintf('voronoin for periodic cells is not supported') 
        return 
    end
end
...
[dd, stdd] = datadensity(x,y,method,radius,blnIsParagrm,lenA,lenB,angle);
%------------- Gridding -------------------
...
return
%~~~~~~~~~~ Data Density ~~~~~~~~~~~~~~
function [dd, stdd] = datadensity(x,y,method,r,blnIsParagrm,lenA,lenB,angle)
...
dd = zeros(Ld,1);
if blnIsParagrm
    % expand x,y arrays for searching
    upx = x-lenB*cosd(angle);
    upy = y-lenB*sind(angle);
    dwnx = x+lenB*cosd(angle);
    dwny = y+lenB*sind(angle);
    lftx = x-lenA;
    lfty = y;
    rtx = x+lenA;
    rty = y;
    uplftx = x-lenA-lenB*cosd(angle);
    uplfty = upy;
    uprtx = x+lenA-lenB*cosd(angle);
    uprty = upy;
    dwnlftx = x-lenA+lenB*cosd(angle);
    dwnlfty = dwny;
    dwnrtx = x+lenA+lenB*cosd(angle);
    dwnrty = dwny;

    exx = [x;upx;dwnx;lftx;rtx;uplftx;uprtx;dwnlftx;dwnrtx];
    exy = [y;upy;dwny;lfty;rty;uplfty;uprty;dwnlfty;dwnrty];
    switch method %Calculate Data Density
    case 'sq'  %---- Using squares ----
        for k=1:Ld
            dd(k) = sum( exx>(x(k)-r) & exx<(x(k)+r) & exy>(y(k)-r) & exy<(y(k)+r) );
        end %for
        area = (2*r)^2;
        dd = dd/area;
        stdd = length(x)/(lenA*lenB*sind(angle));
    case 'ci'
        for k=1:Ld
            dd(k) = sum( sqrt((exx-x(k)).^2 + (exy-y(k)).^2) < r );
        end
        area = pi*r^2;
        dd = dd/area;
        stdd = length(x)/(lenA*lenB*sind(angle));
...
return
%~~~~~~~~~~ Graf Scatter Plot ~~~~~~~~~~~
function varargout = gsp(x,y,c,ms,uplim)
%Graphs scattered poits
map = colormap(jet);
if isempty(uplim) || uplim == 0
    ind = fix((c-min(c))/(max(c)-min(c))*(size(map,1)-1))+1;
else
    % from 0 to uplim instead of from min(c) to max(c)
    ind = fix((c-0)/(uplim-0)*(size(map,1)-1))+1;
    ind(ind>size(map,1)) = size(map,1); % fix upper boundary
end
h = [];
